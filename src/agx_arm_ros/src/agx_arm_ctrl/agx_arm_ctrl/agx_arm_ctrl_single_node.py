...
class AgxArmRosNode(Node):
    ...
    def _load_parameters(self):
        ...
        # Periodic hardware enable sync (P1)
        self._last_enable_sync_time = 0.0
        self._enable_sync_interval = 1.0  # seconds between hardware enable polls
        # is_ok() failure tracking (P3)
        self._consecutive_not_ok = 0
        self._not_ok_warned = False
        # move command failure throttling (P2)
        self._last_move_fail_warn_time = 0.0
    ...
    def _log_parameters(self):
        ...
        self.get_logger().info(f"home_joint_positions: {self.home_joint_positions}")
    ...
    def _init_agx_arm(self):
        ...
        if self.firmware:
            ...
            if firmeware_version != PiperFW.DEFAULT:
                ...
                self.agx_arm = AgxArmFactory.create_arm(config)
                self.agx_arm.connect()
                # Re-enable after firmware-triggered reconnect, otherwise the
                # arm stays disabled because disconnect() triggers a hardware-
                # level safety stop on the Piper firmware side.
                if self.auto_enable:
                    if not self._enable_arm(True, self.enable_timeout):
                        self.get_logger().error(
                            "Failed to auto-enable the arm after firmware reconnect"
                        )
        else:
    ...
    def _create_pose_cmd(self, pose: Pose) -> list:
        ...
        # Send TCP coordinates directly — the arm driver already knows the TCP
        # offset via set_tcp_offset().  Calling get_tcp2flange_pose() here
        # would apply the offset twice, producing unreachable targets that
        # trigger firmware safety stops.
    ...
    def _home_joint_target(self):
        if len(self.home_joint_positions) == self.arm_joint_count:
            return list(self.home_joint_positions)
        self.get_logger().warn(
            f"home_joint_positions length {len(self.home_joint_positions)} "
            f"does not match arm joint count {self.arm_joint_count}; using zeros"
        )
        return [0.0] * self.arm_joint_count
    ...
    def _enable_arm(self, enable: bool = True, timeout: float = 5.0) -> bool:
        ...
        return all_joints_in_target_status

    ### publisher thread
    def _sync_hardware_enable(self):
        """Poll the actual joint enable status from hardware and reconcile with
        cached ``enable_flag``.  Call periodically from the publish thread so
        that firmware-triggered disables (safety stop, collision, comm loss)
        are detected without waiting for an explicit service call."""
        try:
            hw_enabled = self.agx_arm.get_joint_enable_status(255)
        except Exception:
            return  # hardware not responding yet; try again next cycle
        if hw_enabled != self.enable_flag:
            prev = self.enable_flag
            self.enable_flag = hw_enabled
            if prev and not hw_enabled:
                self.get_logger().error(
                    "Hardware enable LOST — arm disabled externally "
                    "(safety stop / firmware protection). "
                    "Control commands will be blocked until re-enabled."
                )
            elif not prev and hw_enabled:
                self.get_logger().info(
                    "Hardware enable RESTORED — arm re-enabled externally."
                )
    ...
    def _publish_thread(self):
        ...
        # publishing loop
        while rclpy.ok():
            arm_ok = self.agx_arm.is_ok()
            if arm_ok:
                # P3: arm recovered from not-ok state
                if self._consecutive_not_ok > 0:
                    self.get_logger().info(
                        f"Agx_arm recovered after {self._consecutive_not_ok} "
                        "consecutive not-ok cycles"
                    )
                self._consecutive_not_ok = 0
                self._not_ok_warned = False
                ...
                if not self.control_ready and self._check_arm_ready():
                    ...
                    if not self._control_ready_logged:
                        ...
                        self.get_logger().info(
                            f"Agx_arm feedback is ready, external control gate is {gate_state}"
                        )
                        self._control_ready_logged = True

                # P1: periodic hardware enable sync
                now_sec = time.time()
                if now_sec - self._last_enable_sync_time >= self._enable_sync_interval:
                    self._last_enable_sync_time = now_sec
                    self._sync_hardware_enable()
                ...
                # P3: arm not ok — track and warn
                self._consecutive_not_ok += 1
                if self._consecutive_not_ok >= 20 and not self._not_ok_warned:
                    self._not_ok_warned = True
                    self.get_logger().error(
                        f"Agx_arm.is_ok() returned False for "
                        f"{self._consecutive_not_ok} consecutive cycles "
                        f"(~{self._consecutive_not_ok / max(self.pub_rate, 1):.1f}s). "
                        "Feedback publishing suspended."
                    )
            rate.sleep()
    ...
    def _publish_gripper_status(self):
        ...
        if status is not None:
            ...
            msg.width = float(status.width)
            msg.force = float(status.force)
            msg.voltage_too_low = bool(status.voltage_too_low)
            msg.motor_overheating = bool(status.motor_overheating)
            msg.driver_overcurrent = bool(status.driver_overcurrent)
            msg.driver_overheating = bool(status.driver_overheating)
            msg.sensor_status = bool(status.sensor_status)
            msg.driver_error_status = bool(status.driver_error_status)
            msg.driver_enable_status = bool(status.driver_enable_status)
            msg.homing_status = bool(status.homing_status)
    ...
    def _handle_move_result(self, success: bool, cmd_name: str):
        """Throttled warning when a move command is rejected by the arm."""
        if success:
            return
        now_sec = time.time()
        if now_sec - self._last_move_fail_warn_time < 2.0:
            return
        self._last_move_fail_warn_time = now_sec
        self.get_logger().warn(
            f"{cmd_name} rejected by arm — target may be unreachable "
            "(IK failure, joint limit, or arm not enabled)"
        )
    ...
    def _control_arm_joints(self, joint_pos):
        ...
        if arm_joints:
            ...
            if self.fast_mode:
                ok = self.agx_arm.move_js(joints)
                self._handle_move_result(ok, "move_js")
                self.is_mit_mode = True
            else:
                ok = self.agx_arm.move_j(joints)
                self._handle_move_result(ok, "move_j")
                self.is_mit_mode = False
    ...
    def _move_p_callback(self, msg: PoseStamped):
        ...
        ok = self.agx_arm.move_p(pose_cmd)
        self._handle_move_result(ok, "move_p")
        self.is_mit_mode = False
    ...
    def _move_l_callback(self, msg: PoseStamped):
        ...
        pose_cmd = self._create_pose_cmd(msg.pose)
        ok = self.agx_arm.move_l(pose_cmd)
        self._handle_move_result(ok, "move_l")
        self.is_mit_mode = False
    ...
    def _move_c_callback(self, msg: PoseArray):
        ...
        ok = self.agx_arm.move_c(pose_start, pose_mid, pose_end)
        self._handle_move_result(ok, "move_c")
        self.is_mit_mode = False
    ...
    def _move_mit_callback(self, msg: MoveMITMsg):
        ...
        for i in range(len(msg.joint_index)):
            ...
            ok = self.agx_arm.move_mit(**params)
            self._handle_move_result(ok, f"move_mit(joint={msg.joint_index[i]})")
        self.is_mit_mode = True
    ...
    def _move_home_callback(self, request, response):
        try:
            if not self._check_arm_ready():
                ...
                if self.is_mit_mode:
                    self.agx_arm.move_js(self._home_joint_target())
                else:
                    self.agx_arm.move_j(self._home_joint_target())
