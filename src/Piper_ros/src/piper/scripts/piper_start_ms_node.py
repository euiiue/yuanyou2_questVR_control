...
class C_PiperRosNode():
    ...
    def __init__(self) -> None:
        ...
        self.control_speed = int(rospy.get_param("~control_speed", 20))
        self.control_speed = max(1, min(100, self.control_speed))
        self.command_interval = rospy.Duration(1.0 / max(float(rospy.get_param("~command_hz", 15.0)), 1.0))
        self.last_pos_command_time = rospy.Time(0)
        self.last_joint_command_time = rospy.Time(0)
        self.log_joint_commands = bool(rospy.get_param("~log_joint_commands", False))
        rospy.loginfo("%s control_speed=%s command_hz=%.1f", self.can_port, self.control_speed, 1.0 / self.command_interval.to_sec())
        # publish
    ...
    def _too_soon(self, command_type):
        now = rospy.Time.now()
        if command_type == "pos":
            if now - self.last_pos_command_time < self.command_interval:
                return True
            self.last_pos_command_time = now
            return False

        if now - self.last_joint_command_time < self.command_interval:
            return True
    ...
    def pos_callback(self, pos_data):
        ...
        Args:
            pos_data (): 
        """
        if self._too_soon("pos"):
            return
        if self.log_joint_commands:
            rospy.loginfo("Received PosCmd x=%.3f y=%.3f z=%.3f roll=%.3f pitch=%.3f yaw=%.3f gripper=%.3f",
                          pos_data.x, pos_data.y, pos_data.z, pos_data.roll, pos_data.pitch, pos_data.yaw, pos_data.gripper)
        ...
        if(self.GetEnableFlag()):
            ...
            self.piper.MotionCtrl_2(0x01, 0x02, self.control_speed,0xad)
            ...
            self.piper.MotionCtrl_2(0x01, 0x00, self.control_speed,0xad)
    ...
    def joint_callback(self, joint_data):
        ...
        factor1 = 57.32484
        if self._too_soon("joint"):
            return
        if len(joint_data.position) < 7:
            return
        if self.log_joint_commands:
            rospy.loginfo("%s joint command: %s", self.can_port, ["%.3f" % p for p in joint_data.position[:7]])
        ...
        if(self.GetEnableFlag()):
            self.piper.MotionCtrl_2(0x01, 0x01, self.control_speed,0xad)
